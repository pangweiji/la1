...
<table class="table table-hover">
	...
	<tbody>
		...
		<tr>
			...
			<td>未授权</td>
			<td>
				<button gmail="{{ $l->email }}" platform="{{ $l->platform }}" id="auth" class="btn btn-primary">Gmail授权</button>
			</td>
		</tr>
		@endforeach
	</tbody>
...
<script type="text/javascript">
	...
	$('#auth').click(function () {
		if (subWindow == null || subWindow.closed) {
			...
			subWindow = window.open("{{ url('home/gmail_auth') }}?gmail="+gmail+"&platform="+platform, "", "top=150px,left=350px,width=600px,height=450px,location=yes,menubar=no,resizable=yes,scrollbars=yes,status=no,toolbar=no,z-look=yes")
		} else {
	...
	$('#btnSubmit').click(function() {
		...
		//ajax提交
		$.ajax({
			url: '{{ url(config("laraadmin.homeRoute")."/gmail_list") }}',
			type: 'POST',
			dataType: 'json',
