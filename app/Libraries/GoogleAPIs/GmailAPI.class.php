...
class GmailAPI{
	...
	private $qString	=	'';
    /**
     * Email 优先级 (1 = High, 3 = Normal, 5 = low).
     * @var int
     */
    public $Priority          = 3;
	...
	private   function setClient(){
		...
		$this->client->setApplicationName('gmail');
...
//		$this->client->addScope("https://www.googleapis.com/auth/gmail.readonly");
    ...
    public function refreshToken($refreshToken){
        $this->client->getAuth()->refreshToken($refreshToken);
    }

	private function setConfig($appName){
		$this->authGmail();
		$this->client->setAuthConfigFile(__DIR__.'/'.$appName.'.json');
		
	}

    //set a flag to protect request forgery and embled some info
    public function setState($state){
        $this->client->getAuth()->setState($state);
    }
	...
	public function setQueryInInbox(){
        $this->addQueryIn('inbox');
	}

    public function setQueryInTrash(){
        $this->addQueryIn('trash');
    }

    public function setQueryInSpam(){
        $this->addQueryIn('spam');
    }

    private function addQueryIn($zone){
        if(strpos($this->qString,'in:') !== FALSE){
            $this->qString  .=   ' OR in:'.$zone;
        }else{
            $this->qString  .=   ' in:'.$zone;
        }
    }

	public function setQueryTime($hour){
			$this->qString	.=	 ' newer_than:'.$hour.'h';		
	}
	
	/*
	 * $time 格式 2004/04/16
	 */
	public function setQueryBeforeTime($time){
		$this->qString	.=	 ' before:'.$time;
	}
	
	/*
	 * $time 格式 2004/04/16
	*/
	public function setQueryAfterTime($time){
		$this->qString	.=	 ' after:'.$time;
	}
	
	public function setQueryUnread(){
		$this->qString  .= 	' is:unread ';
	}
	
	public function setQueryRead(){
		$this->qString  .= 	' is:read ';
	}
	
	public function setQueryStarted(){
		$this->qString  .= 	' is:starred ';
	}
	
	public function setQuerySubject($subject){
		$this->qString  .= 	' subject:'.$subject;
	}
	
	public function setQueryLabel($label){
		$this->qString  .= 	' label:'.$label;
	}
	...
	public function listMessageIds($gmail){
        $page   =   1;
		$pageToken = NULL;
		$messages = array();
		$opt_param = array('q'=>$this->qString);
		echo $this->qString."\r\n";
		do {
			try {
				if ($pageToken) {
				...
				$messagesResponse = $this->service->users_messages->listUsersMessages($gmail, $opt_param);
                if($page > 20){
                    $info   =   date('Y-m-d H:i:s').' '.getmypid().' '.$pageToken.var_dump($messagesResponse)."\r\n";
                    file_put_contents(WEB_PATH.'log/gmail_loop.log',$info,FILE_APPEND);
                    break;
                }
	...
	private function modifyMessage($userId, $messageId, $labelsToAdd, $labelsToRemove) {
	...
	public function setUnRead($userId, $messageId){
		$labelsToAdd		=		array('UNREAD');
		$message  			=  $this->modifyMessage($userId, $messageId, $labelsToAdd, array());
		return $message;
	}
	public function setRead($userId, $messageId){
		$labelsToRemove		=		array('UNREAD');
		$message  			=  $this->modifyMessage($userId, $messageId, array(), $labelsToRemove);
		return $message;
	}
	...
	public function getAttachment($userId, $messageId,$attachmentId){
	    $optParams = array();
	    try{
	        $attachment     		=      $this->service->users_messages_attachments->get($userId,$messageId,$attachmentId,$optParams);
	        $attachment_data    =  	$attachment->getData();
	        $attachment_data    =       strtr($attachment_data,'-_', '+/');
	        return base64_decode($attachment_data);
	    } catch (Exception $e) {
	        print 'An error occurred: ' . $e->getMessage();
	    }
	}
	
	public function getEmail($gmail, $msgid,$atPath=null){
        $msg_res = $this->getMessage($gmail, $msgid);
        if(!$msg_res){
            return false;
        }
        $email = [];
        $modelData    = $msg_res->getModelData();
        $payload      = $modelData['payload'];

        $headerInfo   = $payload['headers'];
        foreach($headerInfo as $item){
            switch($item['name']){
                case 'Subject':
                    ...
                    break;
                case 'Date'   :
                    $sendTime = preg_replace('/\(.*\).*$/', '', $item['value']);
                    $sendTime = strtotime($sendTime);
                    $sendTime = empty($sendTime) ? $item['value'] : $sendTime;
                    break;
                case 'From'   :
                    $sender = $item['value'];
                    if(preg_match("/(.*)<(.*)>/", $sender, $res)){
                        $senderName = $res[1];
                        $senderId   = $res[2];
                    }else{
                        $senderName = '';
        ...
        $contentInfo = $this->getContentInfo($payload);
        ...
        $attachments = $contentInfo['attachment'];

        if($atPath && $attachments){
            foreach($attachments as $at){
                $attachment = $atPath.$at['name'];
                $data   = $this->getAttachment($gmail, $msgid, $at['id']);
                $result = file_put_contents($attachment, $data);
            }
        }
        return $email;
    ...
    private function getContentInfo($payload){
        $mailMimeType = $payload['mimeType'];
        $body         = $payload['body'];
        $contentPart     = array();
        $attachmentPart  = array();
        $tmpContentParts = array();
        $counter         = 0;
        if(preg_match('/multipart/', $mailMimeType)){
            $parts = $payload['parts'];//该部分包含邮件的内容和附件信息,第一个元素为邮件内容,其余为附件信息
            if(!isset($parts[0]['partId'])){//事实证明multipart/mixed并不能证明一定包含附件
                $tmpContentParts = $parts[0]['parts'];
                while(true){
                    if($counter++ > 4){
                        break;
                    }
                    if(isset($tmpContentParts[0]['partId'])){
                        $contentPart = $tmpContentParts;
                        break;
                    }
                    $tmpContentParts = $tmpContentParts[0]['parts'];
                }

                unset($parts[0]);//因为$parts[0]是邮件内容部分
            ...
            $plainContent = '';
            $htmlContent  = '';
            foreach($contentPart as $part){
                $mimeType   = $part['mimeType'];
                $tmpContent = $part['body']['data'];
                $tmpContent = strtr($tmpContent, '-_', '+/');
                $tmpContent = base64_decode($tmpContent);
                if($mimeType == 'text/plain'){
                    $plainContent = $tmpContent;
                }elseif($mimeType == 'text/html'){
                    $htmlContent = $tmpContent;
                }
            }

            $content     = empty($htmlContent) ? '<pre>'.$plainContent.'</pre>' : $htmlContent;
            $attachments = array();

            if(!empty($attachmentPart)){
                foreach($attachmentPart as $part){
                    $attachments[] = array('name' => $part['filename'], 'id' => $part['body']['attachmentId']);
                }
            }
        }else{
            $attachments  = array();
        ...
        return ['content'=>$content,'plaincontent'=>$plainContent,'attachment'=>$attachments];
    }
	
	public function listLabels($userId) {
		 $labels = array();
		 try {
     ...
     * @return bool
     */
    public function getSendRawMessage() {
        try {
            $this->mailHeader = "";
     ...
     */
    public function setBody($message, $basedir = '') {
    ...
    public function sendEmail(){
        ...
        $rawMessage     =       $this->getSendRawMessage();;
        $rawMessage     =       base64_encode($rawMessage);
        $rawMessage     =       str_replace(array('+','/','='),array('-','_',''),$rawMessage); // url safe
        $message->setRaw($rawMessage);
        file_put_contents(WEB_PATH.'/log/amazon/send_raw_msg.log',$rawMessage);
        try{
            $sentMsgObj     =       $this->service->users_messages->send($this->From,$message);
        }catch (Exception $e){
            file_put_contents(WEB_PATH.'/log/amazon/sendException.log',date('Y-m-d H:i:s', time()).' '.$e->getMessage(),FILE_APPEND);
            echo $e->getMessage();
        }
